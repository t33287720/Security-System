...
def main():
    ...
    while True:
        ...
        # 3️⃣ ES index update
        # ==================================================
        index_changed = update_index_if_needed(state, ES_HOST, ES_USER, ES_PASS)

        # ── 每日觸發 eval_metrics.py（日期切換 或 超過 24h 未執行）
        _now = datetime.utcnow()
        _eval_due = (
            index_changed                                              # 日期切換
            or (_last_eval is None)                                    # 從未執行過（啟動後首次）
            or ((_now - _last_eval).total_seconds() >= 86400)         # 超過 24 小時
        )
        if _eval_due:
            print(f"[EVAL] 觸發 eval_metrics.py（index_changed={index_changed}, last_eval={_last_eval}）")
            try:
                subprocess.Popen(
