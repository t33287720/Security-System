...
if ($search !== '') {
    $where[] = "(target LIKE CONCAT('%', ?, '%') OR cve_id LIKE CONCAT('%', ?, '%') OR title LIKE CONCAT('%', ?, '%'))";
    $params[] = $search;
    $params[] = $search;
    $params[] = $search;
    $types .= 'sss';
...
// ─── 總筆數 ────────────────────────────────────────────────────
$stmt = $conn->prepare("SELECT COUNT(*) FROM vuln_findings $where_sql");
...
// ─── 分頁資料 ──────────────────────────────────────────────────
$data_sql = "
    SELECT id, target, port, service, version, source, cve_id, title,
